...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    for (i = 0; i < 5; i++) {
      rtb_TmpSignalConversionAtToFile[i + 6] = heli_q8_B.Integrator[i];
    }

    /* End of SignalConversion: '<S8>/TmpSignal ConversionAtTo FileInport1' */

    /* ToFile: '<S8>/To File' */
    if (rtmIsMajorTimeStep(heli_q8_M)) {
      {
        if (!(++heli_q8_DW.ToFile_IWORK.Decimation % 1) &&
            (heli_q8_DW.ToFile_IWORK.Count*12)+1 < 100000000 ) {
          ...
          if (fp != (NULL)) {
            ...
            if (fwrite(u, sizeof(real_T), 12, fp) != 12) {
              rtmSetErrorStatus(heli_q8_M,
                                "Error writing to MAT-file test3_4_3.mat");
              return;
            }

            if (((++heli_q8_DW.ToFile_IWORK.Count)*12)+1 >= 100000000) {
              (void)fprintf(stdout,
                            "*** The ToFile block will stop logging data before\n"
                            "    the simulation has ended, because it has reached\n"
                            "    the maximum number of elements (100000000)\n"
                            "    allowed in MAT-file test3_4_3.mat.\n");
            }
          }
        }
      }
  ...
  tmp[4] = heli_q8_X.Integrator_CSTATE_m;
  ...
  for (i = 0; i < 2; i++) {
    pitch = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      pitch += heli_q8_P.K[(i_0 << 1) + i] * tmp[i_0];
    }
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    {
      t_error result = 0;
      t_boolean close_flag = (heli_q8_P.Constant_Value_b != 0);
  ...
  /* Sum: '<S8>/Sum2' */
  tmp_2[0] = heli_q8_B.pitch_elevation[0];
  tmp_2[1] = heli_q8_B.euler_rates[0];
  tmp_2[2] = heli_q8_B.pitch_elevation[1];
  tmp_2[3] = heli_q8_B.euler_rates[1];
  tmp_2[4] = heli_q8_B.euler_rates[2];
  for (i = 0; i < 5; i++) {
    /* Gain: '<S8>/A' incorporates:
     *  Sum: '<S8>/Sum3'
     */
    tmp[i] = 0.0;

    /* Gain: '<S8>/B' incorporates:
     *  Sum: '<S8>/Sum3'
     */
    tmp_1[i] = 0.0;
    tmp_1[i] += heli_q8_P.B2[i] * rtb_Sum1[0];
    tmp_1[i] += heli_q8_P.B2[i + 5] * rtb_Sum1[1];

    /* Gain: '<S8>/C' incorporates:
     *  Sum: '<S8>/Sum2'
     */
    tmp_3[i] = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
       ...
       *  Sum: '<S8>/Sum3'
       */
      tmp[i] += heli_q8_P.A2[5 * i_0 + i] * heli_q8_B.Integrator[i_0];
  ...
  for (i = 0; i < 5; i++) {
    pitch = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      pitch += heli_q8_P.L[5 * i_0 + i] * tmp_4[i_0];
    }

    heli_q8_B.Sum3[i] = (tmp[i] + tmp_1[i]) + pitch;
  }
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    if (rtmIsMajorTimeStep(heli_q8_M)) {
      {
        if (!(++heli_q8_DW.ToFile_IWORK_p.Decimation % 1) &&
            (heli_q8_DW.ToFile_IWORK_p.Count*2)+1 < 100000000 ) {
          FILE *fp = (FILE *) heli_q8_DW.ToFile_PWORK_k.FilePtr;
          if (fp != (NULL)) {
            real_T u[2];
            heli_q8_DW.ToFile_IWORK_p.Decimation = 0;
            u[0] = heli_q8_M->Timing.t[1];
            u[1] = 0.0;
            if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
              rtmSetErrorStatus(heli_q8_M,
            ...
            if (((++heli_q8_DW.ToFile_IWORK_p.Count)*2)+1 >= 100000000) {
              (void)fprintf(stdout,
                            "*** The ToFile block will stop logging data before\n"
                            "    the simulation has ended, because it has reached\n"
                            "    the maximum number of elements (100000000)\n"
    ...
    /* Sum: '<S7>/Sum3' */
    heli_q8_B.Sum3_p = heli_q8_B.PitchCounttorad - rtb_Gain;
  }
...
void heli_q8_derivatives(void)
{
  ...
  for (i = 0; i < 5; i++) {
    _rtXdot->Integrator_CSTATE[i] = heli_q8_B.Sum3[i];
  }

  /* End of Derivatives for Integrator: '<S8>/Integrator' */

  /* Derivatives for Integrator: '<S7>/Integrator1' */
  _rtXdot->Integrator1_CSTATE = heli_q8_B.Sum3_p;

  /* Derivatives for Integrator: '<S7>/Integrator' */
  _rtXdot->Integrator_CSTATE_m = heli_q8_B.Sum2;
}
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "test3_4_3.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M, "Error creating .mat file test3_4_3.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,12,0,"ans")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file test3_4_3.mat");
      return;
    }
  ...
  /* Start for ToFile: '<S4>/To File' */
  {
    ...
    heli_q8_DW.ToFile_IWORK_p.Count = 0;
    heli_q8_DW.ToFile_IWORK_p.Decimation = -1;
    heli_q8_DW.ToFile_PWORK_k.FilePtr = fp;
  }
...

  {
    ...
    heli_q8_X.Integrator_CSTATE_m = heli_q8_P.Integrator_IC_o;
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<S8>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "test3_4_3.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M, "Error closing MAT-file test3_4_3.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M, "Error reopening MAT-file test3_4_3.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 12, heli_q8_DW.ToFile_IWORK.Count, "ans"))
      {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for ans to MAT-file test3_4_3.mat");
      }
  ...
  /* Terminate for ToFile: '<S4>/To File' */
  {
    ...
    if (fp != (NULL)) {
      ...
      if (rt_WriteMat4FileHeader(fp, 2, heli_q8_DW.ToFile_IWORK_p.Count, "ans"))
      {
        rtmSetErrorStatus(heli_q8_M,
      ...
      heli_q8_DW.ToFile_PWORK_k.FilePtr = (NULL);
    }
  }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (2126473569U);
  heli_q8_M->Sizes.checksums[1] = (1221943232U);
  heli_q8_M->Sizes.checksums[2] = (1926979389U);
  heli_q8_M->Sizes.checksums[3] = (1009887854U);
  ...
  heli_q8_M->Sizes.numBlockPrms = (531);/* Sum of parameter "widths" */
  return heli_q8_M;
}
